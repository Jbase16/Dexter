#!/usr/bin/env bash
# scripts/live-ui-recovery-model-stability.sh - repeat the focused
# model-driven UI recovery smoke and write a stability receipt.
...
STABILITY_DIR="$SUMMARY_DIR/ui-recovery-model-stability"
STAMP="$(date '+%Y%m%d_%H%M%S')"
LOG_DIR="$STABILITY_DIR/logs/$STAMP"
SUMMARY_FILE="$STABILITY_DIR/ui-recovery-model-stability-$STAMP.md"
LATEST_FILE="$STABILITY_DIR/latest.md"
RUNS="${DEXTER_UI_RECOVERY_MODEL_STABILITY_RUNS:-5}"
...
if ! [[ "$RUNS" =~ ^[1-9][0-9]*$ ]]; then
    say FAIL "DEXTER_UI_RECOVERY_MODEL_STABILITY_RUNS must be a positive integer (got '$RUNS')"
    exit 2
fi
...
say INFO "running model-driven UI recovery smoke $RUNS time(s)"
...
for ((run = 1; run <= RUNS; run++)); do
    ...
    say INFO "$run_label: make live-smoke-ui-recovery-model"
    (
        cd "$ROOT_DIR" || exit 2
        make live-smoke-ui-recovery-model
    ) 2>&1 | tee "$log_file"
    status="${PIPESTATUS[0]}"
...
{
    echo "# Dexter UI Recovery Model Stability"
    echo
    ...
    echo
    echo '```bash'
    echo "DEXTER_UI_RECOVERY_MODEL_STABILITY_RUNS=$RUNS make live-smoke-ui-recovery-model-stability"
    echo '```'
} > "$SUMMARY_FILE"
